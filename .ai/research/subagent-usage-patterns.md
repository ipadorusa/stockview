...
## 12. 실전 적용 교훈 (2026-04-08)

### 12.1 모델 배치: 분석=Opus, 수정=Sonnet

**발견**: 첫 실전 적용에서 분석을 Sonnet, 수정을 Opus로 배치했으나 이는 반대가 맞음.

| 단계 | 적합 모델 | 이유 |
|------|----------|------|
| **분석** (패턴 파악, 이슈 분류, 도메인 추론) | **Opus** | 복잡한 추론, 12개 파일 간 연관성 파악 |
| **수정** (분석 결과 기반 코드 편집) | **Sonnet** | 명확한 지시 → 기계적 수정, 비용 효율적 |

리서치 §7의 3-tier 전략과 일치:
```
Haiku (분류/트리아지) → Sonnet (실행/수정) → Opus (분석/리뷰)
```

**적용 방법**: 서브에이전트 정의에서 분석용은 `model: opus`, 실행용은 `model: sonnet` 사용.

### 12.2 커스텀 에이전트 자동화 호출 한계 & 해결

**발견**: `.claude/agents/*.md`에 정의한 커스텀 에이전트는 `@agent-name` 채팅 멘션으로만 동작. Agent tool의 `subagent_type` 파라미터로는 호출 불가.

**영향**: 자동화 워크플로우(Hook 트리거, 스크립트 연동)에서 커스텀 에이전트를 직접 사용할 수 없음.

**해결 방안**:

| 방안 | 방식 | 장단점 |
|------|------|--------|
| **A. 래퍼 스킬** (권장) | `.claude/skills/invoke-agent/SKILL.md` 생성. 에이전트 파일을 Read → 시스템 프롬프트 추출 → general-purpose에 주입 | 자동화 가능, 격리 유지, 한 단계 간접 |
| **B. 직접 주입** | Agent tool 호출 시 prompt에 에이전트 시스템 프롬프트를 수동 복사 | 즉시 가능, 유지보수 어려움 |
| **C. Skill 전환** | `.claude/agents/` → `.claude/skills/`로 이동 | Skill tool로 호출 가능, 단 격리 컨텍스트 상실 |

**결론**: A 방안 (래퍼 스킬)이 격리 + 자동화를 모두 만족하는 최적 해법.
